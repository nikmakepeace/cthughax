...
The Frame Generator migration items have been completed and erased from
this remaining-work plan. The generator path now has owned frame storage,
Frame Generator naming, explicit logging, explicit render context, native Scene
visual selections, native catalog loaders, and boundary tests that reject the
legacy global, `Video*`, `EffectControl`, image-option, palette-entry, and
startup-config compatibility surfaces.

The only related work left here is Display cleanup, which remains outside the
Frame Generator dependency path now that frames are handed to Display explicitly.

1. **Finish the separate Display cleanup outside Frame Generator. Status:
   related remaining.**
